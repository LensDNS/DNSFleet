...
进程内 **漂移检测**、**`POST /sync`** 与 **`GET /api/v1/nodes/:id/querylog`** **共用**同一个 semaphore，上限为 `DNSFLEET_SYNC_MAX_CONCURRENT`（默认 **8**），即任意时刻经该槽位发往 AdGH 的飞行中请求总数不超过该值（含浏览器拉取历史 querylog）。详见根目录 `README.md`「Configuration」。
